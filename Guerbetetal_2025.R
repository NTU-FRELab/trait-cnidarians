...
#FOcc indices
coords <- as.matrix(fspace$sp_faxes_coord[, c("PC1", "PC2")])  # 2D example
species_list <- unique(species)

hulls <- list()

# Step 1: compute convex hull per species
for (sp in species_list) {
  sp_coords <- coords[species == sp, ]
  if (nrow(sp_coords) >= 3) {
    ch <- chull(sp_coords)
    coords_hull <- sp_coords[c(ch, ch[1]), ]
    hulls[[sp]] <- st_polygon(list(as.matrix(coords_hull)))}}

# Step 2: union all species hulls (accounts for overlaps)
all_hulls <- st_sfc(hulls, crs = NA_crs_)
union_hull <- st_union(all_hulls)

# Step 3: total community convex hull
total_ch <- chull(coords)
coords_total_hull <- coords[c(total_ch, total_ch[1]), ]
total_hull <- st_polygon(list(as.matrix(coords_total_hull)))

# Step 4: compute areas
area_union <- st_area(union_hull)
area_total <- st_area(st_sfc(total_hull, crs = NA_crs_))

# Step 5: overall functional occupancy (%)
FO_convex <- as.numeric(area_union / area_total * 100)
cat("Functional Occupancy (Convex Hull):", round(FO_convex, 2), "%\n")

#FOcc indices using Monte Carlo approach
# Step 1: total convex hull
total_hull <- convhulln(coords)  # 2D convex hull of all points

# Step 2: generate random points inside convex hull
n_points <- 100000  # number of points to sample
inside_points <- matrix(NA, nrow = n_points, ncol = 2)
count <- 0

set.seed(42)
while(count < n_points){
  # Generate candidate points in bounding box of all points
  candidate <- cbind(
    runif(n_points, min(coords[,1]), max(coords[,1])),
    runif(n_points, min(coords[,2]), max(coords[,2])))
  
  # Keep only points inside total hull
  inside <- inhulln(total_hull, candidate)
  
  # Add points to the final set
  n_add <- min(sum(inside), n_points - count)
  if(n_add > 0){
    inside_points[(count+1):(count+n_add), ] <- candidate[inside, ][1:n_add, ]
    count <- count + n_add}}

rand_points <- inside_points

# Step 3: check which points fall inside any species hull
inside_any <- rep(FALSE, n_points)
for (sp in species_list) {
  sp_coords <- coords[species == sp, ]
  if (nrow(sp_coords) >= 3) {  # minimum 3 points for 2D hull
    sp_hull <- convhulln(sp_coords)
    inside_any <- inside_any | inhulln(sp_hull, rand_points)}}

# Step 4: estimate functional occupancy
FO_montecarlo <- mean(inside_any) * 100  # percent
cat("Functional Occupancy (Monte Carlo):", round(FO_montecarlo, 2), "%\n")
...
#Order
trait_space_cluster= funspace(pca.trait, PCs = c(1,2), group.vec = comp$cluster,
                            n_divisions = 300)
summary(trait_space_cluster)
plot(x = trait_space_cluster,
     quant.plot = TRUE,
     globalContour = T, # The contour of the global TPD
     pnt = T, # add points for species of each family
     pnt.cex = 0.8, # points should be small
...
# 1. Define clusters and pastel colors
clusters <- unique(comp$cluster)
cluster_colors <- c("pastelblue" = "#7da15d", "pastelgreen" = "#715da1")
names(cluster_colors) <- clusters
point_colors <- cluster_colors[as.character(comp$cluster)]

# 2. Define 13 shapes for species (pch values from 0–25)
species <- comp$species
unique_species <- unique(species)
species_shapes <- c(16, 17, 15, 18, 19, 0, 1, 2, 5, 6, 8, 9, 10)  # 13 different shapes
names(species_shapes) <- unique_species
point_shapes <- species_shapes[species]

# 3. Plot functional space
plot(
  x = trait_space_cluster,
  quant.plot = TRUE,
  globalContour = TRUE,
  pnt = TRUE,
  pnt.cex = 0.8,
  pnt.col = point_colors,
  pnt.pch = point_shapes
)
# 5. Add legend for species (shapes)
legend(
  "bottomright",                  # position
  legend = unique_species,
  pch = species_shapes,
  col = "black",                   # shapes only
  bty = "n"
)
