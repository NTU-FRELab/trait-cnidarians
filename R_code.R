...
#########TRAIT SPACE ######### 
funspaceDim(comp[,5:17]) #retain the first 3 components
pca.trait=princomp(comp[,5:17], cor = TRUE)
...
#Permanova
# Data Preparation
scaled_data <- scale(comp[,5:17])  # Standardize excluding species column
...
#########FUNCTIONAL DIVERSITY INDICES ON mFD#########
trait_data=comp[,c(-2,-3,-4)]
...
#########CLUSTER#########
df <- scale(comp[,5:17]) # Scaling the data
df=as.matrix(df)
pca_result <- prcomp(df, scale. = TRUE)

# Extract PCA scores
pca_scores <- as.data.frame(pca_result$x)
pca_scores$Sample <- rownames(data)
...
#plot k=6
km.res6 <- kmeans(na.omit(df),6, nstart = 50)

res.pca <- prcomp(na.omit(df),  scale = TRUE)
ind.coord <- as.data.frame(get_pca_ind(res.pca)$coord)
ind.coord$cluster <- factor(km.res6$cluster)
ind.coord$Species <- na.omit(comp)$species
eigenvalue <- round(get_eigenvalue(res.pca), 1)
variance.percent <- eigenvalue$variance.percent
head(eigenvalue)
...
#########TRAITS BY TRAITS PLOT AND ANALYSIS#########
###Delta15N
# Step 1: Max Delta13C per species
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(Delta15N))
# Step 2: Permutation Kruskal-Wallis test
kruskal_perm <- adonis2(comp$Delta15N ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(Delta13C))
kruskal_perm <- adonis2(comp$Delta13C ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(ratio.C.N))
kruskal_perm <- adonis2(comp$ratio.C.N ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(Z.dens))
kruskal_perm <- adonis2(comp$Z.dens ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(C.dens))
kruskal_perm <- adonis2(comp$C.dens ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(chla))
kruskal_perm <- adonis2(comp$chla ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(chlb))
kruskal_perm <- adonis2(comp$chlb ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(chlc))
kruskal_perm <- adonis2(comp$chlc ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(chlca))
kruskal_perm <- adonis2(comp$chlca ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(Host.prot.cont))
kruskal_perm <- adonis2(comp$Host.prot.cont ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(Tot.Lip.cont))
kruskal_perm <- adonis2(comp$Tot.Lip.cont ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(In.cont))
kruskal_perm <- adonis2(comp$In.cont ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
value_max2 <- comp %>%
  group_by(sp) %>%
  summarize(max_value = max(Sk.dens))
kruskal_perm <- adonis2(comp$Sk.dens ~ comp$sp, permutations = 999)
print(kruskal_perm)
...
########FUNSPACE############
funspaceDim(comp[,5:16]) #retain the first 3 components
pca.trait=princomp(comp[,5:16], cor = TRUE)
...
#########PERMANOVA###########
# Data Preparation
scaled_data <- scale(comp[,5:16])  # Standardize excluding species column
# Dissimilarity Matrix
dist_matrix <- vegdist(scaled_data, method = "euclidean")
# Test region
...
######mDFD indices#######
comp_cat<-read.table("comp_cat.txt", header = T, sep = "", dec = ".")
trait_data=comp[,-1:-3]
...
##########PERMUTATION WILCOXON TEST + BOXPLOT##########
